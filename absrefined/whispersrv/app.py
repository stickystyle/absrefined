...
app = FastAPI(title="Local Whisper API", description="A local implementation of OpenAI's audio transcription API")
...
async def transcribe(
    ...
    logger.debug(f"Parameters: language={language}, prompt={'present' if prompt else 'none'}, temperature={temperature}")
    ...
    try:
        ...
        result = mlx_whisper.transcribe(temp_path, word_timestamps=True, path_or_hf_repo="mlx-community/whisper-turbo")
                
        logger.debug(f"Initial transcription successful with keys: {result.keys() if isinstance(result, dict) else 'not a dict'}")
