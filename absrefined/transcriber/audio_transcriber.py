...
class AudioTranscriber:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = openai_config.get("openai_api_url") # For custom OpenAI-compatible endpoints

        if not self.api_key:
            raise KeyError("OpenAI API key (openai_api_key) not found in [refiner] config section.")
        ...
        self.whisper_model = openai_config.get("whisper_model_name", "whisper-1") # Default to whisper-1
    ...
    def transcribe_audio(
        ...
        if self.debug_preserve_files:
            ...
            debug_transcript_path = os.path.join(debug_dir, f"{audio_filename_base}_transcript_DEBUG.jsonl")
            self.logger.info(f"Debug mode: Raw OpenAI response segments will be saved to {debug_transcript_path}")
        ...
        if write_to_file and output_file is None:
            ...
            raise ValueError("output_file must be provided if write_to_file is True and not just for debugging.")

        try:
            self.logger.info(f"Transcribing {audio_file} using model '{self.whisper_model}' via {self.base_url or 'OpenAI'}.")
            self.logger.debug(f"Applying time offset: {segment_start_time:.2f}s")
            if write_to_file and output_file:
                self.logger.debug(f"User-specified output transcript will be saved to {output_file}")
            ...
            if self.debug_preserve_files and debug_transcript_path:
                try:
                    with open(debug_transcript_path, "w", encoding="utf-8") as dbg_f:
                        ...
                        if hasattr(response, 'segments'):
                            raw_data_to_save['segments'] = [
                                seg.model_dump() for seg in response.segments # Use model_dump for Pydantic objects
                            ]
                    ...
                    self.logger.info(f"Raw OpenAI response data saved to {debug_transcript_path}")
                except Exception as e_dbg:
                    self.logger.warning(f"Could not save debug transcript to {debug_transcript_path}: {e_dbg}")

            # --- Process the response (segments and words) ---
            openai_segments_raw = response.segments if hasattr(response, 'segments') else []
            openai_words_raw = response.words if hasattr(response, 'words') else []

            if not openai_segments_raw and not openai_words_raw:
                 self.logger.warning(f"API returned no segments or words for {audio_file}")
                 return []

            adjusted_segments = [] # This will store our final segment list with adjusted times

            if openai_segments_raw:
                self.logger.debug(f"Processing {len(openai_segments_raw)} segments from API response.")
                for segment_raw in openai_segments_raw:
                    ...
                    segment_words_raw = segment_raw.words if hasattr(segment_raw, 'words') and segment_raw.words else []
                    ...
                    if not segment_words_raw and openai_words_raw:
                        ...
                        relevant_top_level_words = [
                            w for w in openai_words_raw 
                            if hasattr(w, 'start') and hasattr(w, 'end') and 
                            w.start >= current_segment_orig_start and w.end <= current_segment_orig_end
                        ]
                        if relevant_top_level_words:
                            segment_words_raw = relevant_top_level_words # Use these instead
                    ...
                    for word_raw in segment_words_raw: # word_raw is a Word object
                        if hasattr(word_raw, 'start') and hasattr(word_raw, 'end'):
                            adj_seg["words"].append({
                                ...
                                "probability": getattr(word_raw, 'probability', None) # v2.0.0 has this as `probability`
                            })
                        else:
                             self.logger.warning(f"Word object missing start/end time: {word_raw}")
                 ...
                 self.logger.warning(f"API returned words but no segments for {audio_file}. Creating one synthetic segment.")
                 ...
                 for word_raw in openai_words_raw:
                     if hasattr(word_raw, 'start') and hasattr(word_raw, 'end'):
                         ...
                         self.logger.warning(f"Word object missing start/end time in top-level list: {word_raw}")
                 ...
                 if all_adj_words_for_synthetic_segment:
                     ...
                     self.logger.debug(f"Created synthetic segment from words: {min_s:.2f}s - {max_e:.2f}s")
                 else:
                     self.logger.warning(f"Could not create synthetic segment for {audio_file}, no valid words found.")
                     return []
            
            if not adjusted_segments:
                 self.logger.error(f"Failed to produce any adjusted segments for {audio_file} from API response.")
                 return []
            ...
            if write_to_file and output_file:
                ...
                try:
                    with open(final_output_path, "w", encoding="utf-8") as f:
                        ...
                        disable_tqdm = self.logger.getEffectiveLevel() >= logging.WARNING
                        
                        for segment_to_write in tqdm(adjusted_segments, 
                                                   desc=f"Writing to {os.path.basename(final_output_path)}", 
                                                   disable=disable_tqdm):
                            json.dump(segment_to_write, f)
                            f.write("\n")
                    self.logger.info(f"Adjusted transcription segments saved to {final_output_path}")
                except Exception as e_write:
                    self.logger.error(f"Failed to write transcription to {final_output_path}: {e_write}")
        ...
        # Specific OpenAI errors handled by the SDK and re-raised if not caught by these:
            ...
            self.logger.error(f"OpenAI API Authentication Error: {e}. Check your API key and organization if applicable.")
            raise # Re-raise to be caught by caller
        except RateLimitError as e:
            self.logger.error(f"OpenAI API Rate Limit Error: {e}. Check your usage and limits.")
            raise
        except APIConnectionError as e:
            self.logger.error(f"OpenAI API Connection Error: {e}. Check network or API endpoint URL.")
            raise
        except Exception as e: # Catch-all for other errors during transcription process
            self.logger.exception(f"Unexpected error during transcription for {audio_file}: {e}")
            raise # Re-raise to be caught by caller
