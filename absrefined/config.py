...
def load_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if not search_path.exists() and not config_path:
        ...
        if alt_path.exists():
            ...
            raise ConfigError(
                f"Configuration file not found at {DEFAULT_CONFIG_PATH.resolve()} or relative path {alt_path.resolve()}"
            )
    ...
    try:
        ...
        config_data["refiner"] = {
            ...
            "model_name": str(
                refiner_config_raw.get("model_name", "gpt-4o-mini")
            ),  # Default model
            "whisper_model_name": str(
                refiner_config_raw.get("whisper_model_name", "whisper-1")
            ),  # Default whisper
        }
        ...
        config_data["transcription"] = {
            ...
            "enable_fallback": bool(
                transcription_config_raw.get("enable_fallback", False)
            ),
            "api_url": str(transcription_config_raw.get("api_url", "")),
            "api_key": str(transcription_config_raw.get("api_key", "")),
            "whisper_model_name": str(
                transcription_config_raw.get("whisper_model_name", "")
            ),
        }
        ...
        config_data["processing"] = {
            "search_window_seconds": int(
                processing_config_raw.get("search_window_seconds", 60)
            ),
            "download_path": str(processing_config_raw.get("download_path", "")),
        }
        if config_data["processing"]["search_window_seconds"] <= 0:
            raise ConfigError(
                "processing.search_window_seconds must be a positive integer."
            )
        ...
        llm_prompt_cost_raw = costs_config_raw.get(
            "llm_refinement_cost_per_million_prompt_tokens"
        )
        llm_completion_cost_raw = costs_config_raw.get(
            "llm_refinement_cost_per_million_completion_tokens"
        )
        transcription_cost_raw = costs_config_raw.get(
            "audio_transcription_cost_per_minute"
        )

        config_data["costs"] = {
            "llm_refinement_cost_per_million_prompt_tokens": float(llm_prompt_cost_raw)
            if llm_prompt_cost_raw is not None
            else 0.0,
            "llm_refinement_cost_per_million_completion_tokens": float(
                llm_completion_cost_raw
            )
            if llm_completion_cost_raw is not None
            else 0.0,
            "audio_transcription_cost_per_minute": float(transcription_cost_raw)
            if transcription_cost_raw is not None
            else 0.0,
        }
        ...
        if (
            config_data["costs"]["llm_refinement_cost_per_million_completion_tokens"]
            < 0
        ):
            raise ConfigError(
        ...
        if config_data["costs"]["audio_transcription_cost_per_minute"] < 0:
            raise ConfigError(
                "costs.audio_transcription_cost_per_minute cannot be negative."
            )
        ...
        raise ConfigError(
            f"Error reading configuration file {search_path.resolve()}: {e}"
        )
