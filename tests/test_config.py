...
VALID_CONFIG_CONTENT = {
    ...
    "logging": {"level": "DEBUG", "debug_files": True},
    "costs": {
...
class TestConfig:
    def test_get_config_success_custom_path(self, tmp_path):
        ...
        assert (
            config["audiobookshelf"]["host"]
            == VALID_CONFIG_CONTENT["audiobookshelf"]["host"]
        )
        assert (
            config["refiner"]["openai_api_key"]
            == VALID_CONFIG_CONTENT["refiner"]["openai_api_key"]
        )
        assert (
            config["processing"]["download_path"]
            == VALID_CONFIG_CONTENT["processing"]["download_path"]
        )
        assert config["logging"]["debug_files"] is True
        assert (
            config["costs"]["audio_transcription_cost_per_minute"]
            == VALID_CONFIG_CONTENT["costs"]["audio_transcription_cost_per_minute"]
        )
    ...
    def test_get_config_default_path_found(self, tmp_path, monkeypatch):
        ...
        )  # DEFAULT_CONFIG_PATH is tricky with complex search
    ...
    def test_get_config_file_not_found(self):
        ...
        with pytest.raises(
            ConfigError
        ) as excinfo:  # Expect ConfigError from load_config
    ...
    def test_get_config_invalid_toml(self, tmp_path):
        ...
        assert (
            "expected '=' after a key" in str(excinfo.value).lower()
        )  # More specific to the error
    ...
    def test_get_config_applies_defaults(self, tmp_path):
        ...
        assert (
            config["refiner"].get("whisper_model_name") == "whisper-1"
        )  # Corrected: it defaults to "whisper-1"
        assert (
            config["processing"].get("search_window_seconds") == 60
        )  # Default from config.py (was 30, check config.py for actual default)
        ...
        assert (
            config["costs"]["llm_refinement_cost_per_million_completion_tokens"] == 0.0
        )
    ...
    def test_get_config_default_search_order(self, tmp_path, monkeypatch):
        ...
        config_data_1 = {
            ...
            "refiner": {
                "openai_api_url": "url1",
                "openai_api_key": "key1_refiner",
                "model_name": "m1",
            },
        }
        with open(config_file_1, "w") as f:
            toml.dump(config_data_1, f)
        ...
        config_data_2 = {
            ...
            "refiner": {
                "openai_api_url": "url_example",
                "openai_api_key": "key_example_refiner",
                "model_name": "model_example",
            },
        }
        with open(example_like_file, "w") as f:
            toml.dump(config_data_2, f)
    ...
    def test_get_config_negative_costs(self, tmp_path):
        """Test ConfigError for negative cost values."""
        # Test negative llm_refinement_cost_per_million_prompt_tokens
        config_with_negative_prompt_cost = {
            "audiobookshelf": {
                "host": "http://localhost:13378",
                "api_key": "test_abs_api_key",
            },
        ...
        config_file = tmp_path / "negative_prompt_cost.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_negative_prompt_cost, f)

        with pytest.raises(ConfigError) as excinfo:
            get_config(config_file)
        assert (
            "costs.llm_refinement_cost_per_million_prompt_tokens cannot be negative"
            in str(excinfo.value)
        )

        # Test negative llm_refinement_cost_per_million_completion_tokens
        config_with_negative_completion_cost = {
            "audiobookshelf": {
                "host": "http://localhost:13378",
                "api_key": "test_abs_api_key",
            },
        ...
        config_file = tmp_path / "negative_completion_cost.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_negative_completion_cost, f)

        with pytest.raises(ConfigError) as excinfo:
            get_config(config_file)
        assert (
            "costs.llm_refinement_cost_per_million_completion_tokens cannot be negative"
            in str(excinfo.value)
        )

        # Test negative audio_transcription_cost_per_minute
        config_with_negative_transcription_cost = {
            "audiobookshelf": {
                "host": "http://localhost:13378",
                "api_key": "test_abs_api_key",
            },
        ...
        config_file = tmp_path / "negative_transcription_cost.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_negative_transcription_cost, f)

        with pytest.raises(ConfigError) as excinfo:
            get_config(config_file)
        assert "costs.audio_transcription_cost_per_minute cannot be negative" in str(
            excinfo.value
        )

    def test_get_config_invalid_log_level(self, tmp_path):
        """Test ConfigError for invalid log level."""
        config_with_invalid_log_level = {
            ...
            "logging": {"level": "INVALID_LEVEL", "debug_files": True},
        }
        config_file = tmp_path / "invalid_log_level.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_invalid_log_level, f)

        with pytest.raises(ConfigError) as excinfo:
            get_config(config_file)
        assert "logging.level must be one of" in str(excinfo.value)

    def test_get_config_invalid_search_window(self, tmp_path):
        """Test ConfigError for invalid search window seconds."""
        config_with_invalid_search_window = {
            "audiobookshelf": {
                "host": "http://localhost:13378",
                "api_key": "test_abs_api_key",
            },
            ...
            "processing": {
                "search_window_seconds": 0,  # Invalid: must be positive
                "download_path": "temp_audio",
            },
        }
        config_file = tmp_path / "invalid_search_window.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_invalid_search_window, f)

        with pytest.raises(ConfigError) as excinfo:
            get_config(config_file)
        assert "processing.search_window_seconds must be a positive integer" in str(
            excinfo.value
        )

        # Test with negative value
        config_with_negative_search_window = {
            "audiobookshelf": {
                "host": "http://localhost:13378",
                "api_key": "test_abs_api_key",
            },
            "processing": {
                "search_window_seconds": -10,  # Invalid: must be positive
            },
        }
        config_file = tmp_path / "negative_search_window.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_negative_search_window, f)
    ...
    def test_get_config_invalid_abs_timeout(self, tmp_path):
        """Test ConfigError for invalid audiobookshelf timeout."""
        ...
        config_file = tmp_path / "invalid_timeout.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_invalid_timeout, f)

        with pytest.raises(ConfigError) as excinfo:
            get_config(config_file)
        assert "audiobookshelf.timeout must be a positive integer" in str(excinfo.value)
        ...
        config_with_negative_timeout = {
            "audiobookshelf": {
                "host": "http://localhost:13378",
                "api_key": "test_abs_api_key",
                "timeout": -30,  # Invalid: must be positive
            }
        }
        config_file = tmp_path / "negative_timeout.toml"
        with open(config_file, "w") as f:
            toml.dump(config_with_negative_timeout, f)
